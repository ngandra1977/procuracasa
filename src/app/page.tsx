    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <a href="#formulario" className="bg-[#FF9500] hover:bg-orange-600 text-white font-medium px-6 py-2 rounded-lg transition-colors">Começar</a>
            </div>
                ...
                <div className="p-6">
                  <form onSubmit={onSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea placeholder="Descreva o tipo de imóvel, orçamento, preferências..." className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-[#FF9500] min-h-[100px]" value={message} onChange={(e) => setMessage(e.target.value)} />
                      {errors.message && <p className="text-red-500 text-sm mt-1">{errors.message}</p>}
                    </div>
                    <button type="submit" disabled={isSubmitting} className="w-full bg-[#FF9500] hover:bg-orange-600 text-white font-semibold py-3 text-lg rounded-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      <section id="como-funciona" className="py-16 lg:py-24 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { num: 1, title: "Conte-nos o que procura", desc: "Preencha o formulário em 1 minuto." },
              { num: 2, title: "Nós pesquisamos por si", desc: "Procuramos imóveis exclusivos para si." },
              { num: 3, title: "Receba opções", desc: "Apresentamos as melhores oportunidades." },
            ].map((step) => (
              <div key={step.num} className="relative">
                <div className="bg-white rounded-2xl p-8 shadow-lg border border-gray-100 h-full">
                  <div className="absolute -top-4 left-8 w-10 h-10 bg-[#FF9500] rounded-full flex items-center justify-center text-white font-bold">{step.num}</div>
                  <h3 className="text-xl font-bold text-[#1F4E79] mb-3 mt-4">{step.title}</h3>
      ...
      <section id="beneficios" className="py-16 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {["Sem perder horas em portais", "Oportunidades exclusivas", "Acompanhamento personalizado", "Grátis, sem compromisso"].map((title, i) => (
              <div key={i} className="bg-gradient-to-br from-orange-50 to-white rounded-2xl p-6 border border-orange-100 hover:shadow-lg transition-shadow">
                <div className="w-12 h-12 bg-orange-100 rounded-xl flex items-center justify-center mb-4">
                  <svg className="w-6 h-6 text-[#FF9500]" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                </div>
            ...
            <div className="grid md:grid-cols-2">
              <div className="p-8 md:p-10">
                ...
                <ul className="space-y-2">
                  {["Passo-a-passo do processo", "Custos e impostos", "Checklist de documentos"].map((item, i) => (
              ...
              <div className="bg-gray-50 p-8 flex flex-col justify-center">
                <a href="/guia-comprar-casa-portugal.pdf" download className="w-full bg-[#FF9500] hover:bg-orange-600 text-white font-semibold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                  Download Direto (PDF)
                </a>
                <p className="text-xs text-gray-500 text-center mt-4">Sem spam. Gratuito.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 lg:py-20 bg-[#1F4E79]">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <a href="#formulario" className="inline-flex items-center gap-2 bg-[#FF9500] hover:bg-orange-600 text-white font-semibold px-8 py-4 text-lg rounded-lg transition-colors">
            Começar Agora<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
          </a>
        </div>
      </section>
      ...
      <footer className="bg-gray-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex gap-4 mb-4">
            <a href="https://instagram.com/procuracasa.pt" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-[#FF9500]"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg></a>
            <a href="https://tiktok.com/@procuracasa" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center hover:bg-[#FF9500]"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z"/></svg></a>
